...
<@security.authorize access="hasAnyRole('A', 'S')">
	...
	<#if clustered>
		<col width="70">
			...
			<tr id="head_tr_id">
				...
				<th class="center nothing" style="padding-left:3px" data-step="4"
					data-intro="<@spring.message 'intro.list.perftest.status'/>"><@spring.message "common.label.status"/></th>
	...
	<#if clustered>
		<th id="region" name="region"><@spring.message "common.region"/></th>
				...
				<th class="nothing" data-step="5"
					data-intro="<@spring.message 'intro.list.perftest.actions'/>"><@spring.message "common.label.actions"/></th>
			</tr>
			</thead>
			<tbody>
...
<script>
	$(document).ready(function () {
		...
		$("#batch_start_btn").click(function () {
			var list = $("td input:checked");
			if (list.length == 0) {
				bootbox.alert("<@spring.message "perfTest.message.delete.alert"/>", "<@spring.message "common.button.ok"/>");
				return;
			...
			bootbox.confirm("<@spring.message "perfTest.message.batch.start.confirm"/>", "<@spring.message "common.button.cancel"/>", "<@spring.message "common.button.ok"/>", function (result) {
				if (result) {
					var ids = list.map(function () {
						return $(this).val();
					}).get();
		...
		$("#saved_only_checkbox, #scheduled_only_checkbox, #running_only_checkbox").click(function () {
			var $this = $(this);
